...
class Worker
{
    ...
    protected static function checkSapiEnv()
    {
        ...
        // DIRECTORY_SEPARATOR 是php的常量 在linux 中表示 '/' 在window中表示 是 '\'
    ...
    protected static function init()
    {
        ...
        // Start file.
        $backtrace        = \debug_backtrace();//这个函数能够找到入口文件
    ...
    {
        //这边的$worker_id是spl_object_hash函数生成的唯一的id
    ...
    protected static function forkOneWorkerForLinux(self $worker)
    {
        ...
        // _idMap数组 键是该worker对象hash生成的唯一id  值应该是存放生成worker进程的元素
    ...
    {
        \set_error_handler(function(){});//设置用户自定义的错误处理函数
        // >=php 5.5
        if (\function_exists('cli_set_process_title')) {
            \cli_set_process_title($title);//Sets the process title visible in tools such as top and ps. This function is available only in CLI mode.
     ...
     * @return void
     */
    //当reload的时候  主进程和子进程都会执行reload的方法
    ...
    public function __construct($socket_name = '', array $context_option = array())
    {
        ...
        $this->workerId                    = \spl_object_hash($this);//spl_object_hash ( object $obj ) : string 本函数为指定对象返回一个唯一标识符。这个标识符可用于作为保存对象或区分不同对象的hash key。
        ...
        $this->_autoloadRootPath = \dirname($backtrace[0]['file']); ///opt/Workerman
        //在Autoloader对象 中保存项目根目录地址
        Autoloader::setRootPath($this->_autoloadRootPath);

        // Context for socket.
        if ($socket_name) {
            $this->_socketName = $socket_name; //http://0.0.0.0:9501
            if (!isset($context_option['socket']['backlog'])) { 
                $context_option['socket']['backlog'] = static::DEFAULT_BACKLOG;
            }
            $this->_context = \stream_context_create($context_option);//创建并返回一个资源流上下文，该资源流中包含了 options 中提前设定的所有参数的值。
        }
